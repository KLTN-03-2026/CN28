...
export const DepositModal = ({
  ...
  const [paymentMethod, setPaymentMethod] = useState<"BANK" | "MOMO" | "VNPAY">(
    "BANK",
  );
  ...
  const paymentMethods: Array<{
    ...
    },
    {
      id: "MOMO",
      name: "MoMo",
      icon: <div className="size-5 bg-pink-500 rounded-lg" />,
    },
    {
      id: "VNPAY",
      name: "VNPay",
      icon: <div className="size-5 bg-blue-600 rounded-lg" />,
    },
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (paymentMethod === "BANK") {
        ...
        const res = await api.post("/api/payment/create-momo-url", {
          amount: val,
        });
      ...
      toast.error(getErrorMessage(err, "Lỗi nạp tiền"));
    } finally {
      ...
      <Modal isOpen={isOpen} onClose={onClose} title="Yêu cầu thành công">
        <div className="text-center space-y-4">
          ...
          <p className="text-smaller text-slate-500">
            Vui lòng thực hiện chuyển khoản theo thông tin đã cung cấp. Số dư sẽ
            được cập nhật sau khi Admin xác nhận.
          </p>
          <button
            onClick={onClose}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {paymentMethods.map((method) => (
            <button
              ...
              className={`p-3 rounded-5 border-2 transition-all flex flex-col items-center gap-2 ${
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-5 py-4 rounded-5 border border-slate-200 dark:border-slate-800 focus:border-primary outline-none transition-all font-black text-xl"
          />
        </div>

        {paymentMethod === "BANK" ? (
          <>
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-4 rounded-5 flex gap-3">
              <AlertCircle className="w-5 h-5 text-amber-600 shrink-0" />
              <p className="text-[11px] text-amber-800 dark:text-amber-400 font-medium leading-relaxed">
                Vui lòng chuyển khoản đúng số tiền và nội dung chuyển khoản là
                Email của bạn.
              </p>
            </div>

            <div className="p-4 rounded-5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 space-y-2">
            ...
            <button
              disabled={loading}
              type="submit"
              className="w-full py-4 bg-primary text-white font-black rounded-5 hover:shadow-xl hover:shadow-primary/20 transition-all flex items-center justify-center gap-2"
            >
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
              <p className="text-smaller text-slate-600 dark:text-slate-400 text-center italic">
                Hệ thống sẽ chuyển hướng bạn đến cổng thanh toán{" "}
                {paymentMethod === "MOMO" ? "MoMo" : "VNPay"} để hoàn tất giao
                dịch.
              </p>
            </div>
            <button
              disabled={loading}
              type="submit"
              className={`w-full py-4 text-white font-black rounded-5 hover:shadow-xl transition-all flex items-center justify-center gap-2 ${
                paymentMethod === "MOMO"
                  ? "bg-[#A50064] hover:shadow-pink-500/20"
                  : "bg-[#005BAA] hover:shadow-blue-500/20"
              }`}
            >
              {loading
                ? "Đang xử lý..."
                : `Thanh toán qua ${paymentMethod === "MOMO" ? "MoMo" : "VNPay"}`}
            </button>
          </div>
        )}
      </form>
...
export const WithdrawModal = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("[WithdrawModal] Submitting:", {
      val,
      balance,
      bankName,
      accountNumber,
    ...
    try {
      ...
      toast.error(getErrorMessage(err, "Lỗi rút tiền"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Rút tiền về ngân hàng">
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30 p-4 rounded-5 flex gap-3">
          <AlertCircle className="w-5 h-5 text-amber-600 shrink-0" />
          <p className="text-[11px] text-amber-800 dark:text-amber-500 font-medium leading-relaxed">
            Yêu cầu tài khoản đã <strong>Approved KYC</strong>. Tiền sẽ được trừ
            khỏi số dư ngay khi bạn nhấn gửi yêu cầu.
          </p>
        </div>

        <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-5 border border-dashed border-slate-200 dark:border-slate-700">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-5 py-4 rounded-5 border border-slate-200 dark:border-slate-800 focus:border-red-500 outline-none transition-all font-black text-xl"
          />
        </div>
        ...
        <button
          disabled={loading}
          type="submit"
          className="w-full py-4 bg-red-600 text-white font-black rounded-5 hover:shadow-xl hover:shadow-red-500/20 transition-all"
        >
