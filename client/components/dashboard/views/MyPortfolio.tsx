...
export default function MyPortfolio({ profile }: { profile: UserProfile }) {
  ...
  const handleDispute = async (projectId: number) => {
    ...
    try {
      await api.post(`/api/projects/${projectId}/disputes`, {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div>
        ...
        <p className="text-slate-600 dark:text-slate-400 text-body mt-1">
          Theo dõi các khoản đầu tư và tiến độ dự án.
        </p>
      </div>
            ...
            <div className="flex flex-col justify-between gap-6">
              <div className="">
                {inv.project?.thumbnailUrl ? (
                  <img
                    src={inv.project.thumbnailUrl}
                    alt=""
                    className="w-full h-80 rounded-xl object-cover shrink-0"
                  />
                ) : (
                  <div className="w-full h-80 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0">
              ...
              <div className="">
                <div className="min-w-0">
                  <h3 className="font-bold text-body text-slate-900 dark:text-white truncate text-base">
                    {inv.project?.title}
                  </h3>
            ...
            <div className="mt-6 pt-6 border-t border-slate-50 dark:border-slate-800/50 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <Link
                  ...
                  className="text-smaller font-bold text-slate-500 hover:text-primary transition-colors flex items-center gap-1"
                >
              ...
              {disputingId === inv.project?.id && (
                <div className="w-full mt-4 bg-red-50 dark:bg-red-900/10 rounded-xl p-4 border border-red-100 dark:border-red-900/20">
                  <p className="text-smaller font-bold text-red-700 dark:text-red-400 mb-3">
                    Khiếu nại dự án
                  </p>
                  <textarea
                    value={disputeReason}
                  ...
                  <div className="flex justify-end gap-2">
                    <button
                      ...
                      className="px-4 py-2 text-smaller font-bold text-slate-500 hover:text-slate-700 transition"
                    >
                      Hủy
                    </button>
                    <button
                      onClick={() => handleDispute(inv.project!.id)}
                      className="px-4 py-2 bg-red-500 text-white text-smaller font-bold rounded-lg hover:bg-red-600 transition shadow-sm"
                    >
                      Gửi khiếu nại
                    </button>
                  </div>
