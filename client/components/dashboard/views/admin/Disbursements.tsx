...
export default function Disbursements() {
  ...
  const {
    ...
    queryFn: async () =>
      (
        await api.get<PendingMilestone[]>(
          "/api/admin/projects/milestones/pending",
        )
      ).data,
  });
  ...
  const approveMilestone = async (projectId: number, milestoneId: number) => {
    if (
      !confirm(
        "Bạn có chắc chắn muốn duyệt giai đoạn này và giải ngân tiền cho chủ dự án?",
      )
    )
      return;
    try {
      await api.post(
        `/api/admin/projects/${projectId}/milestones/${milestoneId}/finalize`,
      );
      toast.success("Giải ngân thành công!");
      refetch();
    } catch (err: unknown) {
      toast.error(getErrorMessage(err, "Không thể thực hiện giải ngân."));
    }
  };
  ...
  const handleReject = async (projectId: number, milestoneId: number) => {
    ...
    try {
      await api.patch(
        `/api/admin/projects/${projectId}/milestones/${milestoneId}/reject`,
        {
      ...
      toast.error(getErrorMessage(err, "Lỗi khi từ chối."));
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div>
        <h1 className="text-h3 font-bold text-slate-900 dark:text-white">
          Duyệt Giải Ngân (Milestones)
        </h1>
        <p className="text-slate-600 dark:text-slate-400 text-body mt-1">
          Kiểm tra bằng chứng và phê duyệt giải ngân vốn cho các dự án theo từng
          giai đoạn.
        </p>
      </div>
            ...
            <div className="md:w-1/3 bg-slate-100 dark:bg-slate-800 relative group aspect-video md:aspect-auto min-h-80">
              {m.proofUrl ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 p-6 text-center">
                  ...
                  <p className="text-smaller font-bold mt-2">
        ...
        {pendingMilestones.length === 0 && (
          <div className="py-20 text-center bg-white dark:bg-slate-900 rounded-5 border border-slate-200 dark:border-slate-800 border-dashed">
            ...
            <p className="text-smaller text-slate-500">
              Hiện tại không có yêu cầu giải ngân nào cần xử lý.
            </p>
          </div>
        )}
      </div>
