...
export default function MyProjectMilestonesPage() {
  ...
  const [activeUploadMilestoneId, setActiveUploadMilestoneId] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const projectRes = await api.get<ProjectDetail>(
          `/api/projects/${params.id}`,
        );
        setProject(projectRes.data);

        if (
          projectRes.data.milestones &&
          projectRes.data.milestones.length > 0
        ) {
          ...
          setMilestones([
            {
              title: "Chi phí nguyên vật liệu mở đầu",
              percentage: 30,
              stage: 1,
            },
          ]);
  ...
  const handleSaveMilestones = async () => {
    ...
    if (total !== 100) {
      setToast({
        type: "error",
        message: `Tổng tỷ lệ bắt buộc phải bằng 100%. Hiện đang là ${total}%`,
      });
      return;
    }
    ...
    try {
      ...
      const updated = await api.get<ProjectDetail>(
      ...
      setToast({
        type: "error",
        message: getErrorMessage(err, "Lưu cấu hình thất bại."),
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleStartVoting = async (milestoneId: number) => {
    ...
    try {
      ...
      setToast({
        type: "error",
        message: getErrorMessage(err, "Không thể bắt đầu bình chọn."),
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleProofSelected = async (url: string) => {
    ...
    try {
      await api.patch(
        `/api/projects/${project.id}/milestones/${activeUploadMilestoneId}/proof`,
        { evidenceUrls: [url] },
      );
      setToast({
        type: "success",
        message: "Đã nộp minh chứng thành công! Chờ Admin duyệt.",
      });
      ...
      setToast({
        type: "error",
        message: getErrorMessage(err, "Không thể nộp minh chứng."),
      });
    }
  };
  ...
  const addMilestone = () => {
    setMilestones([
      ...milestones,
      { title: "Giai đoạn mới", percentage: 0, stage: milestones.length + 1 },
    ]);
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background-light dark:bg-background-dark p-20 animate-pulse text-center">
        Đang tải cấu hình dự án...
      </div>
    );
  }
  ...
  if (!project) {
    return (
      <div className="min-h-screen pt-20 text-center">Dự án không tồn tại!</div>
    );
  }
    ...
    <div className="bg-background-light dark:bg-background-dark min-h-screen font-display pb-20">
      ...
      {toast && (
        <div className="fixed top-20 right-5 z-60">
          <div
            className={`px-4 py-3 rounded-lg shadow-lg text-small font-semibold ${toast.type === "success" ? "bg-green-600" : "bg-red-600"} text-white`}
          >
            {toast.message}
          </div>
        </div>
      ...
      <main className="wrapper wrapper--md py-10">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-2">
              Dự án: <strong className="text-primary">{project.title}</strong>
            </p>
          </div>
          <button
            onClick={() => router.back()}
            className="px-5 py-2 rounded-xl bg-slate-200 dark:bg-slate-800 font-bold hover:bg-slate-300 transition"
          >
            Trở về Dashboard
          </button>
        </div>
        ...
        {isPending ? (
          <div className="bg-white dark:bg-slate-900 rounded-5 border border-slate-200 dark:border-slate-800 p-6 shadow-sm mb-10">
            ...
            <div className="bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-500 p-4 rounded-xl text-small mb-6 border border-amber-200 dark:border-amber-900/30">
              Vì dự án đang ở trạng thái <strong>Draft (Pending)</strong>, bạn
              có thể tự do cấu hình lộ trình giải ngân. Lưu ý tổng tỷ lệ phải
              bằng đúng 100%. Khi dự án được Publish (Funding/Active), cấu hình
              này sẽ bị <strong>KHÓA</strong>.
            </div>
            ...
            <div className="space-y-4">
              {milestones.map((m, idx) => (
                <div
                  key={idx}
                  className="flex gap-4 items-center bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-100 dark:border-slate-800"
                >
              ...
              <button
                ...
                className="w-full border-2 border-dashed border-slate-200 dark:border-slate-800 text-slate-500 rounded-xl py-3 hover:bg-slate-50 hover:text-primary transition font-bold text-small"
              >
                + Thêm Giai Đoạn Mới
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-5 border border-slate-200 dark:border-slate-800 p-6 shadow-sm mb-10">
            ...
            <div className="space-y-4">
              {project.milestones && project.milestones.length > 0 ? (
                project.milestones.map((m: EditableMilestone, idx) => (
                  <div
                    key={idx}
                    className={`p-5 rounded-xl border flex justify-between gap-6 overflow-hidden relative ${
                      m.status === "disbursed"
                        ? "bg-green-50/50 border-green-200"
                        : m.status === "admin_review"
                          ? "bg-amber-50/50 border-amber-200"
                          : m.status === "uploading_proof"
                            ? "bg-primary/5 border-primary/20 shadow-sm border-2"
                            : "bg-slate-50 border-slate-200 opacity-60"
                    }`}
                  >
                    <div className="flex gap-4 items-start relative z-10 w-full">
                      <div
                        className={`size-12 flex items-center justify-center font-bold text-lg rounded-full shrink-0 ${
                          m.status === "disbursed"
                      ...
                      <div className="w-full">
                        <div className="flex justify-between items-center">
                          <h3 className="font-bold text-slate-900 dark:text-slate-900">
                            {m.title}
                          </h3>
                          <p className="font-bold text-lg text-slate-900 dark:text-slate-900">
                            {m.percentage}%
                          </p>
                        </div>
                            ...
                            <div className="mt-4 p-3 bg-red-100 text-red-800 rounded-lg text-small border border-red-200 flex gap-2">
                              ...
                              <div>
                                <p className="font-bold">
                                  Bằng chứng trước đó bị từ chối với lý do:
                                </p>
                                <p className="mt-1">
                                  &quot;{m.rejectionReason}&quot;
                                </p>
                        ...
                        {m.proofUrl && m.status !== "uploading_proof" && (
                          <div className="mt-4 opacity-80 flex items-center gap-2">
                            ...
                            <a
                              href={m.proofUrl}
                              target="_blank"
                              className="font-bold text-small text-green-700 hover:underline"
                            >
                ...
                <p className="text-smaller text-slate-500">
                  Chưa cấu hình Milestones cho dự án này.
                </p>
              )}
            </div>
          </div>
