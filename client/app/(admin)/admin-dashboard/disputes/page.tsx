...
export default function AdminDisputesPage() {
  ...
  const [selectedMilestone, setSelectedMilestone] =
    useState<DisputedMilestone | null>(null);
  ...
  const handleResolveProject = async (
    projectId: number,
    action: "dismiss" | "refund",
  ) => {
    if (!confirm(action === "dismiss" ? "Bỏ qua khiếu nại?" : "HOÀN TIỀN?"))
      return;
    try {
      await api.post(`/api/admin/projects/${projectId}/disputes/resolve`, {
        action,
      });
      fetchData();
    } catch (err: unknown) {
      alert(`Lỗi: ${getErrorMessage(err, "Không thể xử lý khiếu nại.")}`);
    }
  };

  // Milestone Mediation Actions
  const openMediation = async (m: DisputedMilestone) => {
  ...
  const sendFeedback = async () => {
    ...
    try {
      await api.post(
        `/api/admin/projects/milestones/${selectedMilestone.id}/feedback`,
        { content: feedback },
      );
      setFeedback("");
              ...
              projects.map((project) => (
                ...
                >
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <h2 className="font-black text-h6 text-slate-900">
                        {project.title}
                      </h2>
                    ...
                    <div className="flex gap-3">
                      <button
                        onClick={() =>
                          handleResolveProject(project.id, "dismiss")
                        }
                        className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-smaller transition-all"
                      >
                        Bỏ qua
                      </button>
                      <button
                        onClick={() =>
                          handleResolveProject(project.id, "refund")
                        }
                        className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl text-smaller shadow-lg shadow-red-500/20 transition-all"
                      >
                        Hoàn tiền & Đóng
                      </button>
                    </div>
                  ...
                  <div className="bg-red-50 dark:bg-red-900/10 p-4 rounded-5 border border-red-100 dark:border-red-900/30">
                    ...
                    <div className="space-y-2">
                      {project.disputes?.map((d: ProjectDispute) => (
                        <div
                          key={d.id}
                          className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-red-100 dark:border-red-900/20 text-smaller shadow-sm"
                        >
                ...
                milestones.map((m) => (
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-[9px] font-black ${m.status === "admin_review" ? "bg-orange-100 text-orange-600" : "bg-red-100 text-red-600"}`}
                      >
                    ...
                    <h3 className="font-bold text-small line-clamp-1">
                      {m.title}
                    </h3>
                    <p
                      className={`text-[11px] mt-1 ${selectedMilestone?.id === m.id ? "text-white/80" : "text-slate-500"}`}
                    >
                      Dự án: {m.project?.title}
                    </p>
                  </div>
                ))
              )}
              ...
              {selectedMilestone ? (
                <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden animate-in fade-in duration-300">
                  <div className="p-6 bg-slate-900 text-white">
                    <div className="flex justify-between items-start">
                      <div>
                        <h2 className="text-h6 font-black">
                          {selectedMilestone.title}
                        </h2>
                        <p className="text-smaller text-white/60">
                          Giai đoạn {selectedMilestone.stage} •{" "}
                          <strong>{selectedMilestone.project?.title}</strong>
                        </p>
                      </div>
                      <div className="flex gap-2">
                        <button
                          onClick={() => resetVote(selectedMilestone.id)}
                        ...
                        <button
                          onClick={() =>
                            terminateProject(selectedMilestone.project.id)
                          }
                          className="p-2 bg-red-500 hover:bg-red-600 rounded-xl transition-all"
                          title="Hủy dự án"
                        >
                  ...
                  <div className="p-6">
                    ...
                    <div className="space-y-4 mb-6 max-h-160 overflow-y-auto px-2">
                      {discussions.map((d, i) => (
                        <div
                          key={i}
                          className={`flex gap-3 ${d.senderId === selectedMilestone.project.ownerId ? "" : "flex-row-reverse"}`}
                        >
                          <div
                            className={`p-4 rounded-3xl text-smaller max-w-[85%] ${d.senderId === selectedMilestone.project.ownerId ? "bg-slate-100 text-slate-800 rounded-tl-none" : "bg-primary text-white rounded-tr-none shadow-md"}`}
                          >
                            <div className="font-black text-[10px] mb-1 opacity-70">
                              {d.senderId === selectedMilestone.project.ownerId
                                ? "OWNER"
                                : "ADMIN"}
                            </div>
                            {d.content}
                      ...
                      {discussions.length === 0 && (
                        <div className="text-center py-12 text-slate-400 italic text-smaller">
                          Bắt đầu hội thoại để yêu cầu chủ dự án giải trình...
                        </div>
                      )}
                    </div>
                    ...
                    <div className="relative pt-4 border-t border-slate-100">
                      <textarea
                        className="w-full bg-slate-50 rounded-5 p-4 pr-16 text-small min-h-[100px] border-none focus:ring-1 ring-primary outline-none"
                      ...
                      <button
                        ...
                        disabled={!feedback}
                        className="absolute right-4 bottom-8 p-3 bg-slate-900 text-white rounded-5 hover:bg-primary transition-all disabled:opacity-50"
                      >
                ...
                <div className="h-full flex flex-col items-center justify-center bg-white rounded-3xl border border-dashed border-slate-300 p-12 text-center">
                  ...
                  <h3 className="font-black text-slate-400">
                    Chọn một Milestone để bắt đầu điều phối
                  </h3>
                </div>
              )}
            </div>
